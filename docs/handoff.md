...
- **2段配置を実機で確認していない。** 実機検証をしたときに開いていたTerminalが3枚しか
  なく、1段の経路しか通っていない。混在DPIからの整列と復元は`3 of 3`で通っている。
  6枚以上での`N of M`はまだ見ていない。
...
- **復元後、secondary monitor側で物理1pxずれることがある。** 実測で -1497→-1496。
  scriptが扱うDPI非対応座標系では完全一致しており（`Restored 3 of 3`）、
  150%スケールの仮想化による丸め。追うと座標系をDPI-awareにする話になるので触っていない。
